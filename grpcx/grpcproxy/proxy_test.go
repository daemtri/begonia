...
func TestLegacyBehaviour(t *testing.T) {
	...
	testCC, err := backendDialer(t, grpc.WithCodec(grpcproxy.Codec()))
	if err != nil {
		t.Fatal(err)
	}
	...
	go func() {
		...
		proxySrv := grpc.NewServer(
			//lint:ignore SA1019 regression test
			grpc.CustomCodec(grpcproxy.Codec()), // was previously needed for proxy to function.
			grpc.UnknownServiceHandler(grpcproxy.TransparentHandler(directorFn)),
		)
	...
	proxyClient := testservice2.NewTestServiceClient(proxyCC)

	// 4. Run the tests!
	testservice2.TestTestServiceServerImpl(t, proxyClient)
}
...
func TestNewProxy(t *testing.T) {
	...
	go func() {
		...
		// First, we need to create a client connection to this backend.
		proxySrv := grpcproxy.NewProxy(testCC)
...
func backendDialer(t *testing.T, opts ...grpc.DialOption) (*grpc.ClientConn, error) {
	...
	testservice2.RegisterTestServiceServer(testSrv, testservice2.DefaultTestServiceServer)
